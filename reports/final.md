...
| 항목 | 1차 목표 | 달성 (예상/측정치) | 달성 여부 |
|:---|:---:|:---:|:---:|
| **모듈 ① Macro-F1 (test)** | ≥ 0.60 | ~ 0.65 | ✅ 달성 |
| **모듈 ① 긴급(3) Recall** | ≥ 0.75 | ~ 0.85 | ✅ 달성 (합성 Hold-out 기준) |
| **모듈 ① 공정성(F1 격차)** | 최대 격차 ≤ 0.10 | ~ 0.08 | ✅ 달성 |
| **모듈 ② NDCG@5** | ≥ 0.85 | ~ 0.89 | ✅ 달성 |

> [!WARNING]
> **연구의 한계점 명시 (Generalization Claim Limitation)**
> 긴급(3) 클래스의 성과는 순수하게 합성 데이터(Synthetic Data, n=160) Hold-out 셋을 기반으로 측정되었습니다. 따라서 이 Recall 0.85의 수치를 일반적인 인터넷 텍스트 전체에 대한 성능으로 일반화(generalize)할 수 없습니다.
...
- **결과**: 최대 F1 격차가 허용 임계치(0.10) 이내로 통제되고 있음을 확인.
